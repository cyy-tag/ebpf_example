...
/*
	@brief 标记regno编号的寄存器状态为 know zero,
	寄存器值范围全部设置为0
*/
...
/*
	@brief 标记一个寄存器当前存在一个未知值(scalar_value)
*/
...
/*
	@brief 重置寄存器状态和值范围，并标记为未初始化状态
*/
static void __mark_reg_not_init(const struct bpf_verifier_env *env,
				struct bpf_reg_state *reg)
{
	__mark_reg_unknown(env, reg);
	//NOT_INIT 类型表示当前寄存器未初始化
	reg->type = NOT_INIT;
}

/*
  @brief 重置编号为regno的寄存器状态
	@param regs 寄存器状态
	@param regno 寄存器编号
*/
static void mark_reg_not_init(struct bpf_verifier_env *env,
			      struct bpf_reg_state *regs, u32 regno)
{
	/*检查寄存器编号regno 不超过最大值MAX_BPF_REG(10个寄存器)*/
	if (WARN_ON(regno >= MAX_BPF_REG)) {
			   ...
			   struct bpf_func_state *state)
{
	...
	/* frame pointer */
	/*BPF_REG_FP 标记类型type=PTR_TO_STACK R10寄存器值为: 只读栈帧指针*/
...
/*
	@brief 初始化BPF MAIN函数状态
	初始化所有寄存器状态为NOT_INIT，并设置栈帧指针R0（read-only frame pointer to access stack）值为0
*/
...
/* 检查寄存器参数合法性, src 和 dst register */
/* 标记src为可读, 标记dst为可写，并设置为unknow状态 */
			 ...
			 enum reg_arg_type t)
{
	...
	if (t == SRC_OP) {
		...
		if (t == DST_OP)
			mark_reg_unknown(env, regs, regno); /* 目标寄存器标记为unknown状态等待写入值 */
	}
	return 0;
}
...
static int check_alu_op(struct bpf_verifier_env *env, struct bpf_insn *insn)
{
	...
	if (opcode == BPF_END || opcode == BPF_NEG) {
		...
		if (opcode == BPF_NEG) {
			...
			/* 检查BPF_END指令合法性
		   * BPF_END 操作：TO_LE 或者 TO_BE 字节交换 Reserved: do byte swap unconditionally
			 * source value 0--TO_LE 1 -- TO_BE
			 * imm: 表示每次交换字节的宽度 16 32 64
			 */
			if (insn->src_reg != BPF_REG_0 || insn->off != 0 ||
			    (insn->imm != 16 && insn->imm != 32 && insn->imm != 64) ||
			    BPF_CLASS(insn->code) == BPF_ALU64) {
					/* class = insn-code & 0x07 低三位表示操作指令类型 */
		...
		if (BPF_SRC(insn->code) == BPF_X) {
			/*
				BPF_X: 使用源寄存器作为操作源操作对象
				BPF_K: 使用imm 立即数作为操作源操作对象
				使用BPF_X时, insn->imm和insn->off应该都为0
			*/
			...
			/*BPF_K: 使用 imm立即数为源操作对象 检查src_reg合法性 */
			if (insn->src_reg != BPF_REG_0 || insn->off != 0) {
				verbose(env, "BPF_MOV uses reserved fields\n");
				return -EINVAL;
			}
		...
		if (BPF_SRC(insn->code) == BPF_X) {
			...
			/* case: R = imm, 立即数赋值, 寄存器类型为scalar value
		...
		if ((opcode == BPF_MOD || opcode == BPF_DIV) &&
		 ...
		 /* MOD和DIV操作 检查当立即数作为分母是否为0 
		  * src1 % imm or src1 / imm
		  */
